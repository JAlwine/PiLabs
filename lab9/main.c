...
if (++writePosition == write_buf_size){
	write(fd_write, write_buf, sizeof(char)*write_buf_size);
	writePosition = 0;
	wp = &write_buf[0];
}
}
...
write(fd_write, write_buf, sizeof(char)*writePosition);
...
read(fd_read, readbuff, 1);

if (readbuff[0] == 0xFFFFFFFF)
	return -1;
else { 
	temp = readbuff[0];	
	if (temp > 0)
...
while(read_count < read_buf_size){
	temp = read(fd_read, read_buf, read_buf_size);
	read_count +=temp;

	rp = &read_buf[0];
	//printf("New readcount is %d", read_count);
	if (temp == 0)
		return -1;
}
}
